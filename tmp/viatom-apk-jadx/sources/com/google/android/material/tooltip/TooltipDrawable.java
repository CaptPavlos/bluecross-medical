...
public class TooltipDrawable extends MaterialShapeDrawable implements TextDrawableHelper.TextDrawableDelegate {
    ...
    private float calculatePointerOffset() {
        int i10;
        int i11 = ((this.displayFrame.right - getBounds().right) - this.locationOnScreenX) - this.layoutMargin;
        Rect rect = this.displayFrame;
        if (i11 < 0) {
            i10 = ((rect.right - getBounds().right) - this.locationOnScreenX) - this.layoutMargin;
        } else {
    ...
    private EdgeTreatment createMarkerEdge() {
        ...
        float fWidth = (float) ((getBounds().width() - (Math.sqrt(2.0d) * this.arrowSize)) / 2.0d);
    ...
    private void loadFromAttributes(@Nullable AttributeSet attributeSet, @AttrRes int i10, @StyleRes int i11) {
        ...
        if (textAppearance != null) {
            ...
            if (typedArrayObtainStyledAttributes.hasValue(i12)) {
                textAppearance.setTextColor(MaterialResources.getColorStateList(this.context, typedArrayObtainStyledAttributes, i12));
            }
        }
