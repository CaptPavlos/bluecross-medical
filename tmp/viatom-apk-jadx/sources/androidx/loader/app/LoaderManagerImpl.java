...
class LoaderManagerImpl extends LoaderManager {
        ...
        public void onCleared() {
            ...
            while (true) {
                ...
                if (i10 >= size) {
                    ...
                    sparseArrayCompat.valueAt(i10).destroy(true);
                    i10++;
                }
            }
        }
    ...
    public void destroyLoader(int i10) {
        if (this.mLoaderViewModel.isCreatingLoader()) {
            androidx.window.layout.c.g("Called while creating a loader");
            return;
        }
        if (Looper.getMainLooper() != Looper.myLooper()) {
            androidx.window.layout.c.g("destroyLoader must be called on the main thread");
            return;
        }
        if (DEBUG) {
    ...
    public <D> Loader<D> initLoader(int i10, @Nullable Bundle bundle, @NonNull LoaderManager.LoaderCallbacks<D> loaderCallbacks) {
        ...
        if (Looper.getMainLooper() != Looper.myLooper()) {
            androidx.window.layout.c.g("initLoader must be called on the main thread");
            return null;
        }
        LoaderInfo<D> loader = this.mLoaderViewModel.getLoader(i10);
        if (DEBUG) {
    ...
    public <D> Loader<D> restartLoader(int i10, @Nullable Bundle bundle, @NonNull LoaderManager.LoaderCallbacks<D> loaderCallbacks) {
        ...
        if (Looper.getMainLooper() != Looper.myLooper()) {
            androidx.window.layout.c.g("restartLoader must be called on the main thread");
            return null;
        }
        if (DEBUG) {
