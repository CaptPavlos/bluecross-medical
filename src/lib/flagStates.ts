...
import type { FlagState, MedicalRegulationSource, TMASContact } from './types';

type LegacyFlagState = Omit<
  FlagState,
  | 'paris_mou_status'
  | 'paris_mou_rank'
  | 'paris_mou_edition'
  | 'paris_mou_valid_from'
  | 'paris_mou_valid_until'
  | 'paris_mou_inspections'
  | 'paris_mou_detentions'
  | 'paris_mou_source_url'
...
export const OFFICIAL_DOCS = {
  WHO_GUIDE: 'https://iris.who.int/server/api/core/bitstreams/d834ff4a-6e90-4812-b2b1-b89785c30c35/content',
  ...
  MALTA_MEDICAL: 'https://legislation.mt/getpdf/67adafe31b765e1e1c5e8c04',
  PARIS_MOU_CURRENT: CURRENT_PARIS_MOU_EDITION.landing_page_url,
  PARIS_MOU_WHITE_CURRENT: CURRENT_PARIS_MOU_EDITION.sources.find(source => source.status === 'white')?.url ?? CURRENT_PARIS_MOU_EDITION.landing_page_url,
  PARIS_MOU_GREY_CURRENT: CURRENT_PARIS_MOU_EDITION.sources.find(source => source.status === 'grey')?.url ?? CURRENT_PARIS_MOU_EDITION.landing_page_url,
  PARIS_MOU_BLACK_CURRENT: CURRENT_PARIS_MOU_EDITION.sources.find(source => source.status === 'black')?.url ?? CURRENT_PARIS_MOU_EDITION.landing_page_url,
};

// These are legacy medical summaries only. Paris MoU performance is overlaid
// from the versioned official dataset below and is never read from these fields.
const LEGACY_FLAG_STATES: LegacyFlagState[] = [
...
## TMAS Contact
Danish TMAS available 24/7 through Radio Medical services.`, medical_categories: ['A', 'B', 'C'], key_requirements: ['Annual inspection', 'MLC 2006', 'EU Directive 92/29/EEC', 'MC certificate', 'WHO Medical Guide'], inspection_interval_months: 12, maritime_authority: 'Danish Maritime Authority (Søfartsstyrelsen)', authority_website: 'https://dma.dk', published: true, created_at: null, updated_at: null },
  { id: 'no', country_name: 'Norway', country_code: 'NO', flag_emoji: '🇳🇴', paris_mou_status: 'white', paris_mou_rank: 3, slug: 'norway-flag-medical-requirements', excerpt: 'Norway ranks #3 on Paris MOU White List 2024. Norwegian Maritime Authority sets strict Nordic medical standards.', content: '# Norway Flag Medical Requirements\n\nNorway ranks #3 on Paris MOU White List 2024.\n\n## Maritime Authority\n**Norwegian Maritime Authority (Sjøfartsdirektoratet)**\nWebsite: https://sdir.no\n\n## Categories\n- **Category A**: Unlimited, full inventory\n- **Category B**: Within 200nm, reduced\n- **Category C**: Coastal/Fjord\n\n## Requirements\n- 12-month inspection\n- TMAS contact displayed\n- Nordic Medical Standards\n- MLC 2006 compliance', medical_categories: ['A', 'B', 'C'], key_requirements: ['Annual inspection', 'TMAS contact', 'Nordic standards', 'MLC 2006'], inspection_interval_months: 12, maritime_authority: 'Norwegian Maritime Authority', authority_website: 'https://sdir.no', published: true, created_at: null, updated_at: null },
...
- Record keeping per MLC 2006`, medical_categories: ['A1/A2', 'A3', 'A4'], key_requirements: ['Marine Notice MLC-005', 'WHO inventory', 'Annual inspection', 'Worldwide pharmacy acceptance'], inspection_interval_months: 12, maritime_authority: 'LISCR - Liberian Registry', authority_website: 'https://liscr.com', published: true, created_at: null, updated_at: null },
...
function countryNameForDisplay(officialName: string): string {
  ...
  return preferredNames[officialName] ?? officialName.replace(/, UK$/, '');
}

function slugForCountry(countryName: string): string {
  return `${countryName
    .normalize('NFKD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
    .replace(/&/g, ' and ')
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '')}-flag-medical-requirements`;
}
...
const performanceSourceUrls = new Map(
  CURRENT_PARIS_MOU_EDITION.sources.map((source) => [source.id, source.url]),
);
const legacyByIsoCode = new Map(
  LEGACY_FLAG_STATES.map((flag) => [flag.country_code.toUpperCase(), flag]),
);

const listedFlags: FlagState[] = CURRENT_PARIS_MOU_EDITION.entries.map((performance) => {
  const legacy = legacyByIsoCode.get(performance.iso_code);
  const commonPerformance = {
    paris_mou_status: performance.status,
    paris_mou_rank: performance.rank,
    paris_mou_edition: CURRENT_PARIS_MOU_EDITION.edition,
    paris_mou_valid_from: CURRENT_PARIS_MOU_EDITION.valid_from,
    paris_mou_valid_until: CURRENT_PARIS_MOU_EDITION.valid_until,
    paris_mou_inspections: performance.inspections,
    paris_mou_detentions: performance.detentions,
    paris_mou_source_url: performanceSourceUrls.get(performance.source_id)
      ?? CURRENT_PARIS_MOU_EDITION.landing_page_url,
  } satisfies Pick<
    FlagState,
    | 'paris_mou_status'
    | 'paris_mou_rank'
  ...
  if (legacy) {
    return {
      ...legacy,
      ...commonPerformance,
      excerpt: cleanLegacyExcerpt(legacy),
      content: cleanLegacyContent(legacy),
      medical_categories: null,
      key_requirements: [],
      inspection_interval_months: null,
      yacht_requirements: undefined,
      equipment_differences: undefined,
      medical_review_status: 'needs-authoritative-review',
      medical_reviewed_on: null,
      medical_sources: medicalSourcesFor(legacy),
  ...
  const countryName = countryNameForDisplay(performance.display_name);
  return {
    id: performance.iso_code.toLowerCase(),
    country_name: countryName,
    country_code: performance.iso_code,
    flag_emoji: flagEmoji(performance.iso_code),
    ...commonPerformance,
    slug: slugForCountry(countryName),
    excerpt: `${countryName} does not yet have an authoritatively reviewed onboard medical regulation summary in this database.`,
    content: `# ${countryName} Flag Medical Requirements

This entry currently records official Paris MoU performance only. Flag-specific onboard medical requirements have not yet been checked against authoritative sources.

## Before operational use
Confirm the applicable medicine chest, equipment, inspection and crewing requirements with the flag-state maritime authority and a qualified maritime medical supplier.`,
    medical_categories: null,
    key_requirements: [],
    inspection_interval_months: null,
    maritime_authority: null,
    medical_review_status: 'needs-authoritative-review',
    medical_reviewed_on: null,
    medical_sources: [],
    medical_disclaimer: MEDICAL_DISCLAIMER,
...
const notListedFlags: FlagState[] = LEGACY_FLAG_STATES
  .filter((legacy) => !CURRENT_PARIS_MOU_EDITION.entries.some(
    (performance) => performance.iso_code === legacy.country_code,
  ))
  .map((legacy) => ({
    ...legacy,
    paris_mou_status: 'not-listed',
    paris_mou_rank: null,
    paris_mou_edition: CURRENT_PARIS_MOU_EDITION.edition,
    paris_mou_valid_from: CURRENT_PARIS_MOU_EDITION.valid_from,
    paris_mou_valid_until: CURRENT_PARIS_MOU_EDITION.valid_until,
    paris_mou_inspections: null,
    paris_mou_detentions: null,
    paris_mou_source_url: CURRENT_PARIS_MOU_EDITION.landing_page_url,
    excerpt: cleanLegacyExcerpt(legacy),
    content: cleanLegacyContent(legacy),
    medical_categories: null,
    key_requirements: [],
...
/** Current official performance rows followed by retained, unlisted medical summaries. */
export const FLAG_STATES: FlagState[] = [...listedFlags, ...notListedFlags].sort((a, b) => {
  if (a.paris_mou_rank !== null && b.paris_mou_rank !== null) {
    return a.paris_mou_rank - b.paris_mou_rank;
  }
  if (a.paris_mou_rank !== null) return -1;
  if (b.paris_mou_rank !== null) return 1;
  return a.country_name.localeCompare(b.country_name);
});

export const PARIS_MOU_LIST_COUNTS = Object.freeze({
  white: listedFlags.filter((flag) => flag.paris_mou_status === 'white').length,
  grey: listedFlags.filter((flag) => flag.paris_mou_status === 'grey').length,
  black: listedFlags.filter((flag) => flag.paris_mou_status === 'black').length,
  notListed: notListedFlags.length,
});
