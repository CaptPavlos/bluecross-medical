...
public class ChunkSampleStream<T extends ChunkSource> implements SampleStream, SequenceableLoader, Loader.Callback<Chunk>, Loader.ReleaseCallback {
    ...
    public ChunkSampleStream(int i10, int[] iArr, Format[] formatArr, T t, SequenceableLoader.Callback<ChunkSampleStream<T>> callback, Allocator allocator, long j10, int i11, MediaSourceEventListener.EventDispatcher eventDispatcher) {
    ...
    private BaseMediaChunk getLastMediaChunk() {
        return (BaseMediaChunk) a.t(this.mediaChunks, 1);
    }
    ...
    public void discardBuffer(long j10, boolean z9) {
        ...
        while (true) {
            ...
            if (i10 >= sampleQueueArr.length) {
                ...
                sampleQueueArr[i10].discardTo(firstTimestampUs, z9, this.embeddedTracksSelected[i10]);
                i10++;
            }
        }
    }
    ...
    public long getBufferedPositionUs() {
        ...
        if (!lastMediaChunk.isLoadCompleted()) {
            lastMediaChunk = this.mediaChunks.size() > 1 ? (BaseMediaChunk) a.t(this.mediaChunks, 2) : null;
        }
    ...
    public int onLoadError(Chunk chunk, long j10, long j11, IOException iOException) {
        ...
        if (!this.chunkSource.onChunkLoadError(chunk, z10, iOException)) {
            z9 = false;
        } else if (z10) {
            if (zIsMediaChunk) {
        ...
        this.eventDispatcher.loadError(chunk.dataSpec, chunk.type, this.primaryTrackType, chunk.trackFormat, chunk.trackSelectionReason, chunk.trackSelectionData, chunk.startTimeUs, chunk.endTimeUs, j10, j11, jBytesLoaded, iOException, z9);
        if (!z9) {
            return 0;
        }
        this.callback.onContinueLoadingRequested(this);
    ...
    @Override // com.mbridge.msdk.playercommon.exoplayer2.source.SampleStream
    public int readData(FormatHolder formatHolder, DecoderInputBuffer decoderInputBuffer, boolean z9) throws IllegalStateException {
        if (isPendingReset()) {
            return -3;
        }
        int i10 = this.primarySampleQueue.read(formatHolder, decoderInputBuffer, z9, this.loadingFinished, this.decodeOnlyUntilPositionUs);
        if (i10 == -4) {
    ...
    public void seekToUs(long j10) {
        ...
        if (isPendingReset()) {
            ...
            if (baseMediaChunk != null) {
                readPosition = sampleQueue.setReadPosition(baseMediaChunk.getFirstSampleIndex(0));
                this.decodeOnlyUntilPositionUs = Long.MIN_VALUE;
            } else {
                readPosition = sampleQueue.advanceTo(j10, true, (j10 > getNextLoadPositionUs() ? 1 : (j10 == getNextLoadPositionUs() ? 0 : -1)) < 0) != -1;
    ...
    public final class EmbeddedSampleStream implements SampleStream {
        ...
        @Override // com.mbridge.msdk.playercommon.exoplayer2.source.SampleStream
        public final int readData(FormatHolder formatHolder, DecoderInputBuffer decoderInputBuffer, boolean z9) throws IllegalStateException {
            if (ChunkSampleStream.this.isPendingReset()) {
                return -3;
            }
            SampleQueue sampleQueue = this.sampleQueue;
            ChunkSampleStream chunkSampleStream = ChunkSampleStream.this;
            int i10 = sampleQueue.read(formatHolder, decoderInputBuffer, z9, chunkSampleStream.loadingFinished, chunkSampleStream.decodeOnlyUntilPositionUs);
            if (i10 == -4) {
