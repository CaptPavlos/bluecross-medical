package h8;

import com.google.android.gms.internal.measurement.l5;

/* compiled from: r8-map-id-41d83e727936d3330b608d725ba7b7c2e83c0817dc12ceb2aead6fdefac83833 */
/* loaded from: classes3.dex */
public final class r implements c8.b {

    /* renamed from: a, reason: collision with root package name */
    public static final r f8846a = new r();

    /* renamed from: b, reason: collision with root package name */
    public static final e8.f f8847b = a.a.k("kotlinx.serialization.json.JsonNull", e8.h.f7972c, new e8.e[0]);

    @Override // c8.a
    public final Object deserialize(f8.e eVar) {
        eVar.getClass();
        l5.g(eVar);
        if (eVar.decodeNotNullMark()) {
            throw new i8.h("Expected 'null' literal");
        }
        eVar.decodeNull();
        return q.INSTANCE;
    }

    @Override // c8.g, c8.a
    public final e8.e getDescriptor() {
        return f8847b;
    }

    @Override // c8.g
    public final void serialize(f8.f fVar, Object obj) {
        ((q) obj).getClass();
        l5.e(fVar);
        fVar.encodeNull();
    }
}
