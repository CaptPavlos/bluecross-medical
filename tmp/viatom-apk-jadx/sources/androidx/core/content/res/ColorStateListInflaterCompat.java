...
public final class ColorStateListInflaterCompat {
    ...
    */
    ...
    @ColorInt
    private static int modulateColorAlpha(@ColorInt int i10, @FloatRange(from = 0.0d, to = Contrast.RATIO_MIN) float f, @FloatRange(from = 0.0d, to = 100.0d) float f10) {
        boolean z9 = f10 >= 0.0f && f10 <= 100.0f;
        if (f == 1.0f && !z9) {
            return i10;
        }
        int iClamp = MathUtils.clamp((int) ((Color.alpha(i10) * f) + 0.5f), 0, 255);
        if (z9) {
            CamColor camColorFromColor = CamColor.fromColor(i10);
            i10 = CamColor.toColor(camColorFromColor.getHue(), camColorFromColor.getChroma(), f10);
        }
