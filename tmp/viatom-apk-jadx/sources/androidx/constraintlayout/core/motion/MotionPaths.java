...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    public void initPolar(int i10, int i11, MotionKeyPosition motionKeyPosition, MotionPaths motionPaths, MotionPaths motionPaths2) {
        ...
        int i12 = motionKeyPosition.mPositionType;
        if (i12 != 1) {
            float f16 = motionKeyPosition.mPercentX;
            if (i12 != 2) {
                float f17 = Float.isNaN(f16) ? f : motionKeyPosition.mPercentX;
                ...
                if (Float.isNaN(f16)) {
                    ...
                    fMin = Math.min(f11, f10) * motionKeyPosition.mPercentX;
                }
                this.mX = fMin;
                if (Float.isNaN(motionKeyPosition.mPercentY)) {
            ...
            float f27 = motionPaths2.mX;
            float f28 = motionPaths.mX;
            this.mX = a.d(f27, f28, f26, f28);
            if (!Float.isNaN(motionKeyPosition.mPercentY)) {
                f = motionKeyPosition.mPercentY;
            }
            float f29 = motionPaths2.mY;
            float f30 = motionPaths.mY;
            this.mY = a.d(f29, f30, f, f30);
        }
    ...
    public void setView(float f, MotionWidget motionWidget, int[] iArr, double[] dArr, double[] dArr2, double[] dArr3) {
        ...
        if (motion != null) {
            ...
            if (!Float.isNaN(f22)) {
                motionWidget.setRotationZ((float) (Math.toDegrees(Math.atan2(fSin, fCos2)) + f22));
            }
            f11 = fC;
            f12 = fCos;
