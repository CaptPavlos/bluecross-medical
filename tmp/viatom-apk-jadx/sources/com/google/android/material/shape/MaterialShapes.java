...
public final class MaterialShapes {
    ...
    @NonNull
    private static RoundedPolygon customPolygon(@NonNull List<VertexAndRounding> list, int i10, float f, float f10, boolean z9) {
        ArrayList arrayList = new ArrayList();
        repeatAroundCenter(list, arrayList, i10, f, f10, z9);
    ...
    private static void repeatAroundCenter(@NonNull List<VertexAndRounding> list, @NonNull List<VertexAndRounding> list2, int i10, float f, float f10, boolean z9) {
        list2.clear();
        toRadial(list, f, f10);
        ...
        if (z9) {
            ...
            for (int i12 = 0; i12 < i11; i12++) {
                for (int i13 = 0; i13 < list.size(); i13++) {
                    boolean z10 = i12 % 2 != 0;
                    int size = z10 ? (list.size() - 1) - i13 : i13;
                    VertexAndRounding vertexAndRounding = list.get(size);
                    if (size > 0 || !z10) {
                        list2.add(new VertexAndRounding(new PointF((i12 * f12) + (z10 ? (list.get(0).vertex.x * 2.0f) + (f12 - vertexAndRounding.vertex.x) : vertexAndRounding.vertex.x), vertexAndRounding.vertex.y), vertexAndRounding.rounding));
                    }
                }
            }
        } else {
            for (int i14 = 0; i14 < i10; i14++) {
                for (VertexAndRounding vertexAndRounding2 : list) {
                    list2.add(new VertexAndRounding(new PointF((i14 * f11) + vertexAndRounding2.vertex.x, vertexAndRounding2.vertex.y), vertexAndRounding2.rounding));
                }
            }
        }
    ...
    private static List<CornerRounding> toRoundingsList(@NonNull List<VertexAndRounding> list) {
        ...
        for (int i10 = 0; i10 < list.size(); i10++) {
            arrayList.add(list.get(i10).rounding);
        }
        return arrayList;
    }
    ...
    private static float[] toVerticesXyArray(@NonNull List<VertexAndRounding> list) {
        ...
        for (int i10 = 0; i10 < list.size(); i10++) {
            int i11 = i10 * 2;
            fArr[i11] = list.get(i10).vertex.x;
            fArr[i11 + 1] = list.get(i10).vertex.y;
        }
        return fArr;
    }
    ...
    public static class VertexAndRounding {
        ...
        public void toCartesian(float f, float f10) {
            ...
            float fCos = (float) ((Math.cos(pointF.x) * pointF.y) + f);
            PointF pointF2 = this.vertex;
            float fSin = (float) ((Math.sin(pointF2.x) * pointF2.y) + f10);
