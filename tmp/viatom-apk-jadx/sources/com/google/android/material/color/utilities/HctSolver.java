...
public class HctSolver {
        ...
        for (int i10 = 0; i10 < 3; i10++) {
            ...
            if (d11 != d12) {
                if (d11 < d12) {
                    ...
                    iCriticalPlaneBelow = criticalPlaneAbove(trueDelinearized(dArr2[i10]));
                } else {
                    iCriticalPlaneAbove = criticalPlaneAbove(trueDelinearized(d11));
                    iCriticalPlaneBelow = criticalPlaneBelow(trueDelinearized(dArr2[i10]));
                }
                double d13 = dHueOf;
                int i11 = iCriticalPlaneAbove;
                int i12 = iCriticalPlaneBelow;
                double d14 = d13;
                for (int i13 = 0; i13 < 8 && Math.abs(i12 - i11) > 1; i13++) {
                    int iFloor = (int) Math.floor((i11 + i12) / 2.0d);
                    double[] coordinate = setCoordinate(dArr, CRITICAL_PLANES[iFloor], dArr2, i10);
                    double dHueOf2 = hueOf(coordinate);
                    if (areInCyclicOrder(d14, d10, dHueOf2)) {
                        i12 = iFloor;
                        dArr2 = coordinate;
                    } else {
                        d14 = dHueOf2;
    ...
    public static double[][] bisectToSegment(double d8, double d10) {
        ...
        for (int i10 = 0; i10 < 12; i10++) {
            double[] dArrNthVertex = nthVertex(d8, i10);
            if (dArrNthVertex[0] < 0.0d) {
                ...
                if (z9) {
                    if (z10) {
                        ...
                        d11 = d13;
                        if (areInCyclicOrder(d12, dHueOf, d13)) {
                        }
                    }
                    if (areInCyclicOrder(d12, d10, dHueOf)) {
                        z10 = false;
                        d13 = dHueOf;
                        dArr2 = dArrNthVertex;
                    } else {
    ...
    public static double chromaticAdaptation(double d8) {
        ...
        return ((MathUtils.signum(d8) * 400.0d) * dPow) / (dPow + 27.13d);
    }
    ...
    public static int findResultByJ(double d8, double d10, double d11) {
        ...
        while (i10 < 5) {
            double d14 = d12;
            double d15 = dSqrt / 100.0d;
            double d16 = d13;
            double d17 = dSqrt;
            double dPow2 = Math.pow(((d10 == 0.0d || dSqrt == 0.0d) ? 0.0d : d10 / Math.sqrt(d15)) * dPow, 1.1111111111111112d);
            double dPow3 = (Math.pow(d15, (d14 / viewingConditions.getC()) / viewingConditions.getZ()) * viewingConditions.getAw()) / viewingConditions.getNbb();
            double d18 = (((0.305d + dPow3) * 23.0d) * dPow2) / (((108.0d * dPow2) * dSin) + (((dPow2 * 11.0d) * dCos) + (23.0d * ncb)));
            double d19 = d18 * dCos;
            double d20 = d18 * dSin;
            double d21 = dPow3 * 460.0d;
            double[] dArrMatrixMultiply = MathUtils.matrixMultiply(new double[]{inverseChromaticAdaptation(((288.0d * d20) + ((451.0d * d19) + d21)) / 1403.0d), inverseChromaticAdaptation(((d21 - (891.0d * d19)) - (261.0d * d20)) / 1403.0d), inverseChromaticAdaptation(((d21 - (d19 * 220.0d)) - (d20 * 6300.0d)) / 1403.0d)}, LINRGB_FROM_SCALED_DISCOUNT);
            double d22 = dArrMatrixMultiply[0];
            if (d22 < 0.0d) {
                break;
            }
            double d23 = dArrMatrixMultiply[1];
            if (d23 < 0.0d) {
                break;
            }
            double d24 = dArrMatrixMultiply[2];
            if (d24 < 0.0d) {
                break;
            }
    ...
    public static double inverseChromaticAdaptation(double d8) {
        ...
        return Math.pow(Math.max(0.0d, (27.13d * dAbs) / (400.0d - dAbs)), 2.380952380952381d) * MathUtils.signum(d8);
    }
    ...
    public static double[] nthVertex(double d8, int i10) {
        double[] dArr = Y_FROM_LINRGB;
        double d10 = dArr[0];
        double d11 = dArr[1];
        double d12 = dArr[2];
        double d13 = i10 % 4 <= 1 ? 0.0d : 100.0d;
        double d14 = i10 % 2 == 0 ? 0.0d : 100.0d;
        if (i10 < 4) {
    ...
    public static double[] setCoordinate(double[] dArr, double d8, double[] dArr2, int i10) {
        return lerpPoint(dArr, intercept(dArr[i10], d8, dArr2[i10]), dArr2);
    }
