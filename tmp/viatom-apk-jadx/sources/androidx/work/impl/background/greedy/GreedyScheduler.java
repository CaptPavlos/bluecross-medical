...
public class GreedyScheduler implements Scheduler, OnConstraintsStateChangedListener, ExecutionListener {
    ...
    private final Map<WorkGenerationalId, a1> mConstrainedWorkSpecs = new HashMap();
    ...
    private void removeConstraintTrackingFor(WorkGenerationalId workGenerationalId) {
        ...
        synchronized (this.mLock) {
            a1VarRemove = this.mConstrainedWorkSpecs.remove(workGenerationalId);
        }
        if (a1VarRemove != null) {
    ...
    private long throttleIfNeeded(WorkSpec workSpec) {
        ...
        synchronized (this.mLock) {
            try {
                ...
                jMax = (Math.max((workSpec.runAttemptCount - attemptData.mRunAttemptCount) - 5, 0) * WorkRequest.DEFAULT_BACKOFF_DELAY_MILLIS) + attemptData.mTimeStamp;
            } catch (Throwable th) {
                throw th;
            }
        }
