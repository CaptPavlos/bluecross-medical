...
final class CircularIndeterminateAdvanceAnimatorDelegate extends IndeterminateAnimatorDelegate<ObjectAnimator> {
    ...
    private void maybeUpdateSegmentColors(int i10) {
        for (int i11 = 0; i11 < 4; i11++) {
            ...
            if (fractionInRange > 0.0f && fractionInRange < 1.0f) {
                ...
                this.activeIndicators.get(0).color = ArgbEvaluatorCompat.getInstance().evaluate(this.interpolator.getInterpolation(fractionInRange), Integer.valueOf(i13), Integer.valueOf(i14)).intValue();
                return;
            }
        }
    }
    ...
    private void updateSegmentPositions(int i10) {
        ...
        for (int i11 = 0; i11 < 4; i11++) {
            activeIndicator.endFraction = (this.interpolator.getInterpolation(getFractionInRange(i10, DELAY_TO_EXPAND_IN_MS[i11], 667)) * 250.0f) + activeIndicator.endFraction;
            activeIndicator.startFraction = (this.interpolator.getInterpolation(getFractionInRange(i10, DELAY_TO_COLLAPSE_IN_MS[i11], 667)) * 250.0f) + activeIndicator.startFraction;
        }
