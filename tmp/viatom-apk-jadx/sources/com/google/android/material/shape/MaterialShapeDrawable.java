...
public class MaterialShapeDrawable extends Drawable implements TintAwareDrawable, Shapeable {
    ...
    public static class SpringAnimatedCornerSizeProperty extends FloatPropertyCompat<MaterialShapeDrawable> {
        ...
        public SpringAnimatedCornerSizeProperty(int i10) {
            super(a.f(i10, "cornerSizeAtIndex"));
            this.index = i10;
        }
    ...
    private boolean hasCompatShadow() {
        ...
        if (i10 == 1 || materialShapeDrawableState.shadowCompatRadius <= 0) {
            return false;
        }
    ...
    private void maybeDrawCompatShadow(@NonNull Canvas canvas) {
        if (hasCompatShadow()) {
            ...
            if (iWidth < 0 || iHeight < 0) {
                c.g("Invalid shadow bounds. Check that the treatments result in a valid path.");
                return;
            }
    ...
    private void updateShape(int[] iArr, boolean z9) {
        ...
        for (int i10 = 0; i10 < 4; i10++) {
            float cornerSize = this.pathProvider.getCornerSizeForIndex(i10, shapeForState).getCornerSize(boundsAsRectF);
            if (z10) {
                this.springAnimatedCornerSizes[i10] = cornerSize;
            }
            SpringAnimation springAnimation = this.cornerSpringAnimations[i10];
            if (springAnimation != null) {
                springAnimation.animateToFinalPosition(cornerSize);
                if (z10) {
                    this.cornerSpringAnimations[i10].skipToEnd();
                }
            }
        }
        if (z10) {
    ...
    private void updateStrokeShapeAppearanceModels() {
        ...
        while (true) {
            ...
            this.springAnimatedStrokeCornerSizes[i10] = Math.max(0.0f, fArr2[i10] - strokeInsetLength);
            i10++;
        }
    }
    ...
    @Override // android.graphics.drawable.Drawable
    public void setAlpha(@IntRange(from = FabTransformationScrimBehavior.COLLAPSE_DELAY, to = ScatterMapKt.Sentinel) int i10) {
        MaterialShapeDrawableState materialShapeDrawableState = this.drawableState;
        ...
        while (true) {
            ...
            if (i10 >= springAnimationArr.length) {
                ...
                if (springAnimationArr[i10] == null) {
                    springAnimationArr[i10] = new SpringAnimation(this, CORNER_SIZES_IN_PX[i10]);
                }
                this.cornerSpringAnimations[i10].setSpring(new SpringForce().setDampingRatio(springForce.getDampingRatio()).setStiffness(springForce.getStiffness()));
                i10++;
            }
        }
    }
