...
public final class PackageInfoCompat {
    ...
    private static byte[] computeSHA256Digest(byte[] bArr) {
        try {
            ...
            a.l("Device doesn't support SHA256 cert checking", e);
            return null;
        }
    }
    ...
    public static boolean hasSignatures(@NonNull PackageManager packageManager, @NonNull String str, @NonNull @Size(min = 1) Map<byte[], Integer> map, boolean z9) throws PackageManager.NameNotFoundException {
        ...
        for (byte[] bArr2 : setKeySet) {
            if (bArr2 == null) {
                com.google.gson.internal.a.n(a3.a.i("Cert byte array cannot be null when verifying ", str));
                return false;
            }
            Integer num = map.get(bArr2);
            if (num == null) {
                com.google.gson.internal.a.n(a3.a.i("Type must be specified for cert when verifying ", str));
                return false;
            }
            int iIntValue = num.intValue();
            if (iIntValue != 0 && iIntValue != 1) {
                c.h("Unsupported certificate type ", num, " when verifying ", str);
                return false;
            }
        }
        ...
        if (signatures.size() != 0 && map.size() <= signatures.size() && (!z9 || map.size() == signatures.size())) {
            ...
            if (it.hasNext()) {
                ...
                if (iIntValue2 != 0) {
                    if (iIntValue2 != 1) {
                        c.j(num2, "Unsupported certificate type ");
                        return false;
                    }
