...
public class StateListSizeChange {
    ...
    private void addStateSizeChange(@NonNull int[] iArr, @NonNull SizeChange sizeChange) {
        ...
        iArr2[i11] = iArr;
        this.sizeChanges[i11] = sizeChange;
        this.stateCount = i11 + 1;
    }

    @Nullable
    public static StateListSizeChange create(@NonNull Context context, @NonNull TypedArray typedArray, @StyleableRes int i10) throws Resources.NotFoundException {
        int next;
    ...
    @Nullable
    private SizeChangeAmount getSizeChangeAmount(@NonNull TypedArray typedArray, int i10, @Nullable SizeChangeAmount sizeChangeAmount) {
    ...
    private void growArray(int i10, int i11) {
        ...
        System.arraycopy(this.sizeChanges, 0, sizeChangeArr, 0, i10);
    ...
    private void loadSizeChangeFromItems(@NonNull Context context, @NonNull XmlPullParser xmlPullParser, @NonNull AttributeSet attributeSet, @Nullable Resources.Theme theme) throws XmlPullParserException, IOException {
        ...
        while (true) {
            ...
            if (next == 2 && depth2 <= depth && xmlPullParser.getName().equals("item")) {
                ...
                addStateSizeChange(StateSet.trimStateSet(iArr, i10), new SizeChange(sizeChangeAmount));
            }
        }
    }
    ...
    public int getMaxWidthChange(@Px int i10) {
        ...
        for (int i12 = 0; i12 < this.stateCount; i12++) {
            SizeChangeAmount sizeChangeAmount = this.sizeChanges[i12].widthChange;
            SizeChangeType sizeChangeType = sizeChangeAmount.type;
            if (sizeChangeType == SizeChangeType.PIXELS) {
                fMax = Math.max(i11, sizeChangeAmount.amount);
            } else if (sizeChangeType == SizeChangeType.PERCENT) {
                fMax = Math.max(i11, i10 * sizeChangeAmount.amount);
            }
            i11 = (int) fMax;
        }
        return i11;
